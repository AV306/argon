...
import me.av306.argon.config.feature.CommandProcessorGroup;
import me.av306.argon.event.ChatOutputEvent;
...
public class CommandProcessor extends IToggleableFeature
{
    ...
    private ActionResult onChatHudAddMessage( String text )
    {
        ...
        if ( !this.isEnabled )
        {
            ...
            {
                this.sendErrorMessage( "text.argon.commandprocessor.disabled" );
        ...
        // Report command
       Argon.INSTANCE.sendInfoMessage( "text.argon.commandprocessor.report", text );
        ...
        if ( !this.handleStandaloneCommand( name, args ) && !this.handleFeatureCommand( name, args ) )
            this.sendErrorMessage( "text.argon.commandprocesor.unresolvable", name );
    ...
    {
        Command cmd = Argon.INSTANCE.commandRegistry.get( targetId );
    ...
    {
        IFeature feature = Argon.INSTANCE.featureRegistry.get( targetId );
        if ( feature == null ) return false;

        if ( args.length < 1 )
        {
           this.sendErrorMessage( "text.argon.commandprocesor.missing.action" );
            return true;
        }
        ...
        switch( args[0] )
        {
            ...
            case "d", "disable", "off" ->
            {
                ...
                {
                    this.sendErrorMessage( "text.argon.commandprocessor.invalid.command.featurenottoggleable" );
                }
            }

            case "set" ->
            {
                if ( args.length < 3 ) this.sendErrorMessage( "text.argon.commandprocessor.missing.args", 3, args.length );
    ...
    public static char prefixStringToChar( String prefixStr )
    {
        ...
        catch( ArrayIndexOutOfBoundsException oobe )
        {
            ...
            Argon.INSTANCE.LOGGER.warn( "Prefix silently corrected to '!'" );
            prefixChar = '!';
        }
    ...
    public Text getHelpText( String arg )
    {
        ...
        switch( arg )
        {
            case "listf", "listfeatures", "features", "lf", "aliases" ->
            {
                helpText.append( "Registered aliases:\n" )
                        .append(Arrays.toString(Argon.INSTANCE.featureRegistry.keySet()
                                    .toArray( String[]::new )
                            )
                        );
            }
        ...
        helpText.formatted(Argon.INSTANCE.MESSAGE_FORMAT );
