...
import me.av306.argon.config.feature.chat.MultiQuickChatGroup;
import me.av306.argon.event.KeyEvent;
		...
		if ( this.keyBinding.isPressed() && action == GLFW.GLFW_PRESS )
        {
            ...
            if ( !message.isBlank() )
            {
                ...
                if ( message.startsWith( "/" ) )
                    Argon.INSTANCE.client.getNetworkHandler().sendChatCommand( message.replace( "/", "" ) );
                else Argon.INSTANCE.client.getNetworkHandler().sendChatMessage( message );
